...
class TrainingArguments(Seq2SeqTrainingArguments):
    ...
    clean_checkpoint: Optional[bool] = field(
        default=False,
        metadata={}
    )
...
class DataTrainingArguments:
    ...
    prefix_shared: Optional[str] = field(
        default=None,
        metadata={"help": "Defines a dictionary from tasks to the tasks embeddings."},
    )
...
def main():
    ...
    args_parser.add_argument("--prefix_shared", type=str)
    args_parser.add_argument('--clean_checkpoint', action='store_true', default=False)
    ...
    if data_args.prefix_shared is not None:
        model_dict = model.state_dict()
        state_dict = torch.load(data_args.prefix_shared)
        model_dict.update(state_dict)
        model.load_state_dict(model_dict)
        #current_device = model.prefix_shared.device
        #print("current_device",current_device)
        logger.info(f"loading pretrained prefix: \n{state_dict}")
        print("prefix_shared", model.prefix_shared)
    ...
    for idx, exm in enumerate(train_dataset):
        inp, lab , extra = exm["input_ids"], exm["labels"], exm["extra_fields"]
    ...
    ### loading optimizer & lr scheduler ###
    # from transformers.optimization import Adafactor, AdafactorSchedule
    # from torch.optim.lr_scheduler import LambdaLR
    # opt_state_dict = torch.load("/data/users/pjlin/compacter/x/prompt_tuning_tokens_init_30k_adafactor_lr-2_test/t5-base/mrpc/42/checkpoint-30/optimizer.pt", map_location=torch.device("cpu")) 
    # print(opt_state_dict.keys())
    ...
    # print(opt_state_dict["param_groups"][0]["params"])
    # print(opt_state_dict["param_groups"][1]["params"])
    
    # opt_state_dict["param_groups"][0]["params"] = [p for n, p in model.named_parameters() if n in decay_parameters]
    # opt_state_dict["param_groups"][1]["params"] = [p for n, p in model.named_parameters() if n not in decay_parameters]

    # opt = Adafactor(opt_state_dict["param_groups"])
    # opt.load_state_dict(opt_state_dict)
    
    # def _get_constant_lambda(_=None):
    #     return 1
    # lr = LambdaLR(opt, _get_constant_lambda)    
    # lr_state_dict = torch.load('/data/users/pjlin/compacter/x/prompt_tuning_tokens_init_30k_adafactor_lr-2_test/t5-base/mrpc/42/checkpoint-30/scheduler.pt')
    # lr.load_state_dict(lr_state_dict)    
    # opt_set = [opt,lr]
    ### loading optimizer & lr scheduler ###
    ...
    # Delete 
    if training_args.clean_checkpoint:
        for ckpt_path in glob.glob(f"{training_args.output_dir}/checkpoint-*/pytorch_model.bin"):
            logger.info(f"Deleting file {ckpt_path}")
            os.remove(ckpt_path)
